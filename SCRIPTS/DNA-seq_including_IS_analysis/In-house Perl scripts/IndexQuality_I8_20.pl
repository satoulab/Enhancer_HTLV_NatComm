...
for ($i=0 ; $i<@data ; $i++){				###一行ずつ、見ていく。
	...
	if($data[$i][9] =~ /^$ARGV[8]/){		###読まれたインデックス配列が、付けた配列と同じであれば。7(9)番目の標準入力に、付けたインデックスを入れる。
		...
		$number[7]=ord($quality[7])-33;
			...
			if($number[0]>20 and $number[1]>20 and $number[2]>20 and $number[3]>20 and $number[4]>20 and $number[5]>20 and $number[6]>20 and $number[7]>20){	###クオリティ値の設定部分！！！！！！この場合は、8塩基すべて30より高ければ。
